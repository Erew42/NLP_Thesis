        ...
        (
            ...
            [
                ...
                VariableRow(
                    ...
                    "Matched count for a signal dictionary divided by recognized_word_total.",
                    "E7",
                ),
                VariableRow(
...
def _build_story(root: Path) -> list[object]:
    ...
    story.append(
        _paragraph(
            "Where needed, r_i,d denotes daily stock_return, r_m,d denotes daily market_return, VOL_i,d denotes share volume, and L_i denotes recognized_word_total from the LM2011 text scorer rather than the raw token total.",
            styles["body"],
        )
    )
...
postevent_return_volatility = sqrt(mse_resid_i,[6,252])
    ...
    story.append(
        _paragraph(
            "All LM2011 dictionary signals used in the regressions are generated by the same scorer for both full 10-K text and item-level scopes. The denominator contract is crucial: proportions divide by recognized_word_total, not by the raw total token count (E7).",
            styles["body"],
        )
    )
    story.append(_paragraph("4.1 Token counts and recognized-word denominator", styles["h2"]))
    story.append(
        _code_block(
            """
...
signal_prop_i,S = matched_count_i,S / recognized_word_total_i
            """,
            styles,
        )
    )
    story.append(
        _paragraph(
            "Accordingly, total_token_count_full_10k and total_token_count_mda are raw token totals, while token_count_full_10k and token_count_mda are master-dictionary recognized-word totals (E7).",
            styles["body"],
        )
    )
    story.append(
        _paragraph(
            "<i>Economic interpretation.</i> Using recognized_word_total as the denominator normalizes for how much of a document is dictionary-readable English, removing sensitivity to boilerplate, tabular material, XBRL tagging residue, and tokenization artefacts. Two 10-Ks with identical negative-word counts but very different amounts of numerical content receive comparable tone intensities, which is the relevant comparison for cross-sectional regressions.",
            styles["body"],
        )
    )
...
term_weight_i,t =
    ((1 + log(tf_i,t)) / (1 + log(recognized_word_total_i))) * idf_t
