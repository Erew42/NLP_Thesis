...
try:
    ...
    _lm2011_rust = None
    _LSEG_LEDGER_RUST_IMPORT_ERROR: str | None = f"{type(exc).__name__}: {exc}"
else:
    _LSEG_LEDGER_RUST_IMPORT_ERROR = None
...
_LSEG_LEDGER_RUST_METRICS: dict[str, int] = {
    "string_array_json_fast_success": 0,
    "string_array_json_fast_failures": 0,
    "string_array_json_fallbacks": 0,
...
def get_lseg_ledger_rust_accel_metrics() -> dict[str, int | str | bool | None]:
    metrics: dict[str, int | str | bool | None] = dict(_LSEG_LEDGER_RUST_METRICS)
    metrics["rust_accel_available"] = _lm2011_rust is not None
    metrics["rust_accel_import_error"] = _LSEG_LEDGER_RUST_IMPORT_ERROR
    return metrics


def reset_lseg_ledger_rust_accel_metrics() -> None:
    for key in _LSEG_LEDGER_RUST_METRICS:
        _LSEG_LEDGER_RUST_METRICS[key] = 0
...
def _item_ids_json_to_tuple_py(item_ids_json: str) -> tuple[str, ...]:
    return tuple(str(item_id) for item_id in json.loads(item_ids_json))


def _string_array_json_to_tuple_py(array_json: str) -> tuple[Any, ...]:
    return tuple(json.loads(array_json))


def _string_array_json_to_tuple(array_json: str) -> tuple[Any, ...]:
    if _lm2011_rust is not None:
        try:
            values = _lm2011_rust.lseg_string_array_json_values(array_json)
            _LSEG_LEDGER_RUST_METRICS["string_array_json_fast_success"] += 1
            return tuple(str(value) for value in values)
        except Exception:
            _LSEG_LEDGER_RUST_METRICS["string_array_json_fast_failures"] += 1
            _LSEG_LEDGER_RUST_METRICS["string_array_json_fallbacks"] += 1
    else:
        _LSEG_LEDGER_RUST_METRICS["string_array_json_fallbacks"] += 1
    return _string_array_json_to_tuple_py(array_json)


def _item_ids_json_to_tuple(item_ids_json: str) -> tuple[str, ...]:
    if _lm2011_rust is not None:
        try:
            values = _lm2011_rust.lseg_ledger_item_ids_json_values(item_ids_json)
            _LSEG_LEDGER_RUST_METRICS["item_ids_json_fast_success"] += 1
            return tuple(str(value) for value in values)
        except Exception:
            _LSEG_LEDGER_RUST_METRICS["item_ids_json_fast_failures"] += 1
            _LSEG_LEDGER_RUST_METRICS["item_ids_json_fallbacks"] += 1
    else:
        _LSEG_LEDGER_RUST_METRICS["item_ids_json_fallbacks"] += 1
    return _item_ids_json_to_tuple_py(item_ids_json)
...
class RequestLedger:
    ...
    def _item_from_row(self, row: sqlite3.Row) -> LedgerItem:
        return LedgerItem(
            ...
            fields=_string_array_json_to_tuple(str(row["fields_json"])),
    ...
    def _batch_from_row(self, row: sqlite3.Row) -> LedgerBatch:
        ...
        return LedgerBatch(
            ...
            fields=_string_array_json_to_tuple(str(row["fields_json"])),
            parameters=dict(json.loads(row["parameters_json"])),
            item_ids=_item_ids_json_to_tuple(str(row["item_ids_json"])),
            state=str(row["state"]),
            attempt_count=int(row["attempt_count"]),
    ...
    def _item_ids_from_row(self, row: sqlite3.Row) -> tuple[str, ...]:
        return _item_ids_json_to_tuple(str(row["item_ids_json"]))
