...
try:
    ...
    _lm2011_rust = None
    _LSEG_STAGE_AUDIT_RUST_IMPORT_ERROR: str | None = f"{type(exc).__name__}: {exc}"
else:
    _LSEG_STAGE_AUDIT_RUST_IMPORT_ERROR = None
...
_LSEG_STAGE_AUDIT_RUST_METRICS: dict[str, int] = {
    "optional_text_fast_success": 0,
    "optional_text_fast_failures": 0,
    "optional_text_fallbacks": 0,
    "optional_int_fast_success": 0,
    "optional_int_fast_failures": 0,
    "optional_int_fallbacks": 0,
    "optional_mapping_fast_success": 0,
    "optional_mapping_fast_failures": 0,
    "optional_mapping_fallbacks": 0,
    "string_list_fast_success": 0,
    "string_list_fast_failures": 0,
    "string_list_fallbacks": 0,
    "item_ids_json_fast_success": 0,
    "item_ids_json_fast_failures": 0,
    "item_ids_json_fallbacks": 0,
}


def get_lseg_stage_audit_rust_accel_metrics() -> dict[str, int | str | bool | None]:
    metrics: dict[str, int | str | bool | None] = dict(_LSEG_STAGE_AUDIT_RUST_METRICS)
    metrics["rust_accel_available"] = _lm2011_rust is not None
    metrics["rust_accel_import_error"] = _LSEG_STAGE_AUDIT_RUST_IMPORT_ERROR
    return metrics


def reset_lseg_stage_audit_rust_accel_metrics() -> None:
    for key in _LSEG_STAGE_AUDIT_RUST_METRICS:
        _LSEG_STAGE_AUDIT_RUST_METRICS[key] = 0
...
def _normalize_optional_mapping_py(value: Any) -> dict[str, Any] | None:
...
def _normalize_optional_mapping(value: Any) -> dict[str, Any] | None:
    if _lm2011_rust is not None:
        try:
            out = _lm2011_rust.normalize_stage_audit_optional_mapping(value)
            _LSEG_STAGE_AUDIT_RUST_METRICS["optional_mapping_fast_success"] += 1
            return None if out is None else dict(out)
        except Exception:
            _LSEG_STAGE_AUDIT_RUST_METRICS["optional_mapping_fast_failures"] += 1
    _LSEG_STAGE_AUDIT_RUST_METRICS["optional_mapping_fallbacks"] += 1
    return _normalize_optional_mapping_py(value)


def _normalize_optional_int_py(value: Any) -> int | None:
    if value is None:
        return None
    try:
        return int(value)
...
def _normalize_optional_int(value: Any) -> int | None:
    if _lm2011_rust is not None:
        try:
            out = _lm2011_rust.normalize_stage_audit_optional_int(value)
            _LSEG_STAGE_AUDIT_RUST_METRICS["optional_int_fast_success"] += 1
            return None if out is None else int(out)
        except Exception:
            _LSEG_STAGE_AUDIT_RUST_METRICS["optional_int_fast_failures"] += 1
    _LSEG_STAGE_AUDIT_RUST_METRICS["optional_int_fallbacks"] += 1
    return _normalize_optional_int_py(value)


def _normalize_optional_text_py(value: Any) -> str | None:
    if value is None:
        return None
...
def _normalize_optional_text(value: Any) -> str | None:
    if _lm2011_rust is not None:
        try:
            out = _lm2011_rust.normalize_stage_audit_optional_text(value)
            _LSEG_STAGE_AUDIT_RUST_METRICS["optional_text_fast_success"] += 1
            return None if out is None else str(out)
        except Exception:
            _LSEG_STAGE_AUDIT_RUST_METRICS["optional_text_fast_failures"] += 1
    _LSEG_STAGE_AUDIT_RUST_METRICS["optional_text_fallbacks"] += 1
    return _normalize_optional_text_py(value)


def _normalize_string_list_py(value: Any) -> list[str]:
    if not isinstance(value, list):
        return []
    result: list[str] = []
    for entry in value:
        text = _normalize_optional_text_py(entry)
        if text is not None:
            result.append(text)
    return result
...
def _normalize_string_list(value: Any) -> list[str]:
    if _lm2011_rust is not None:
        try:
            out = [str(entry) for entry in _lm2011_rust.normalize_stage_audit_string_list(value)]
            _LSEG_STAGE_AUDIT_RUST_METRICS["string_list_fast_success"] += 1
            return out
        except Exception:
            _LSEG_STAGE_AUDIT_RUST_METRICS["string_list_fast_failures"] += 1
    _LSEG_STAGE_AUDIT_RUST_METRICS["string_list_fallbacks"] += 1
    return _normalize_string_list_py(value)


def _item_ids_json_to_list_py(item_ids_json: str) -> list[Any] | None:
    payload = json.loads(item_ids_json)
    return payload if isinstance(payload, list) else None


def _item_ids_json_to_list(item_ids_json: str) -> list[Any] | None:
    if _lm2011_rust is not None:
        try:
            out = [str(entry) for entry in _lm2011_rust.lseg_ledger_item_ids_json_values(item_ids_json)]
            _LSEG_STAGE_AUDIT_RUST_METRICS["item_ids_json_fast_success"] += 1
            return out
        except Exception:
            _LSEG_STAGE_AUDIT_RUST_METRICS["item_ids_json_fast_failures"] += 1
            _LSEG_STAGE_AUDIT_RUST_METRICS["item_ids_json_fallbacks"] += 1
    else:
        _LSEG_STAGE_AUDIT_RUST_METRICS["item_ids_json_fallbacks"] += 1
    return _item_ids_json_to_list_py(item_ids_json)
...
def _is_tolerable_legacy_finish_backfill_attempt_gap(
    *,
    ...
    try:
        ...
        item_ids = _item_ids_json_to_list(str(batch_row["item_ids_json"]))
