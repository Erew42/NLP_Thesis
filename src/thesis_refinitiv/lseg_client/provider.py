...
try:
    ...
    _lm2011_rust = None
    _LSEG_PROVIDER_RUST_IMPORT_ERROR: str | None = f"{type(exc).__name__}: {exc}"
else:
    _LSEG_PROVIDER_RUST_IMPORT_ERROR = None
...
_LSEG_PROVIDER_RUST_METRICS: dict[str, int] = {
    "classify_error_fast_success": 0,
    "classify_error_fast_failures": 0,
    "classify_error_fallbacks": 0,
    "identifier_list_fast_success": 0,
    "identifier_list_fast_failures": 0,
    "identifier_list_fallbacks": 0,
    "unresolved_identifier_parse_fast_success": 0,
    "unresolved_identifier_parse_fast_failures": 0,
    "unresolved_identifier_parse_fallbacks": 0,
    "coerce_int_fast_success": 0,
    "coerce_int_fast_failures": 0,
    "coerce_int_fallbacks": 0,
    "utf8_record_rows_fast_success": 0,
    "utf8_record_rows_fast_failures": 0,
    "utf8_record_rows_fallbacks": 0,
    "sanitize_headers_fast_success": 0,
    "sanitize_headers_fast_failures": 0,
    "sanitize_headers_fallbacks": 0,
    "session_not_opened_fast_success": 0,
    "session_not_opened_fast_failures": 0,
    "session_not_opened_fallbacks": 0,
    "frame_fingerprint_column_fast_success": 0,
    "frame_fingerprint_column_fast_failures": 0,
    "frame_fingerprint_column_fallbacks": 0,
    "frame_fingerprint_fast_success": 0,
    "frame_fingerprint_fast_failures": 0,
    "frame_fingerprint_fallbacks": 0,
}


def get_lseg_provider_rust_accel_metrics() -> dict[str, int | str | bool | None]:
    metrics: dict[str, int | str | bool | None] = dict(_LSEG_PROVIDER_RUST_METRICS)
    metrics["rust_accel_available"] = _lm2011_rust is not None
    metrics["rust_accel_import_error"] = _LSEG_PROVIDER_RUST_IMPORT_ERROR
    return metrics


def reset_lseg_provider_rust_accel_metrics() -> None:
    for key in _LSEG_PROVIDER_RUST_METRICS:
        _LSEG_PROVIDER_RUST_METRICS[key] = 0
...
def _classify_lseg_error_message_py(message: str) -> tuple[str | None, tuple[str, ...]]:
    unresolved_identifiers = _parse_unresolved_identifiers_py(message)
...
def classify_lseg_error_message(message: str) -> tuple[str | None, tuple[str, ...]]:
    if _lm2011_rust is not None:
        try:
            error_kind, unresolved_identifiers = _lm2011_rust.classify_lseg_error_message_value(message)
            _LSEG_PROVIDER_RUST_METRICS["classify_error_fast_success"] += 1
            return error_kind, tuple(str(value) for value in unresolved_identifiers)
        except Exception:
            _LSEG_PROVIDER_RUST_METRICS["classify_error_fast_failures"] += 1
    _LSEG_PROVIDER_RUST_METRICS["classify_error_fallbacks"] += 1
    return _classify_lseg_error_message_py(message)


def _is_session_not_opened_message_py(message: str) -> bool:
    normalized = message.lower()
...
def _is_session_not_opened_message(message: str) -> bool:
    if _lm2011_rust is not None:
        try:
            out = bool(_lm2011_rust.is_lseg_session_not_opened_message(str(message)))
            _LSEG_PROVIDER_RUST_METRICS["session_not_opened_fast_success"] += 1
            return out
        except Exception:
            _LSEG_PROVIDER_RUST_METRICS["session_not_opened_fast_failures"] += 1
    _LSEG_PROVIDER_RUST_METRICS["session_not_opened_fallbacks"] += 1
    return _is_session_not_opened_message_py(message)
...
def _sanitize_headers_py(headers: dict[str, Any]) -> dict[str, Any]:
...
def _sanitize_headers(headers: dict[str, Any]) -> dict[str, Any]:
    if _lm2011_rust is not None:
        try:
            out = _lm2011_rust.lseg_sanitize_headers(headers)
            _LSEG_PROVIDER_RUST_METRICS["sanitize_headers_fast_success"] += 1
            return dict(out)
        except Exception:
            _LSEG_PROVIDER_RUST_METRICS["sanitize_headers_fast_failures"] += 1
    _LSEG_PROVIDER_RUST_METRICS["sanitize_headers_fallbacks"] += 1
    return _sanitize_headers_py(headers)
...
def _frame_fingerprint_py(frame: pl.DataFrame) -> str | None:
    if frame.height == 0:
...
def _frame_fingerprint(frame: pl.DataFrame) -> str | None:
    if _lm2011_rust is not None:
        try:
            out = _lm2011_rust.lseg_frame_fingerprint_columns(
                frame.columns,
                [frame.get_column(column).to_list() for column in frame.columns],
            )
            _LSEG_PROVIDER_RUST_METRICS["frame_fingerprint_column_fast_success"] += 1
            return None if out is None else str(out)
        except Exception:
            _LSEG_PROVIDER_RUST_METRICS["frame_fingerprint_column_fast_failures"] += 1
            _LSEG_PROVIDER_RUST_METRICS["frame_fingerprint_column_fallbacks"] += 1
        try:
            out = _lm2011_rust.lseg_frame_fingerprint_rows(frame.to_dicts())
            _LSEG_PROVIDER_RUST_METRICS["frame_fingerprint_fast_success"] += 1
            return None if out is None else str(out)
        except Exception:
            _LSEG_PROVIDER_RUST_METRICS["frame_fingerprint_fast_failures"] += 1
    _LSEG_PROVIDER_RUST_METRICS["frame_fingerprint_fallbacks"] += 1
    return _frame_fingerprint_py(frame)


def _coerce_int_py(value: Any) -> int | None:
    if value is None:
        return None
    try:
        return int(value)
...
def _coerce_int(value: Any) -> int | None:
    if _lm2011_rust is not None:
        try:
            out = _lm2011_rust.normalize_optional_int_value(value)
            _LSEG_PROVIDER_RUST_METRICS["coerce_int_fast_success"] += 1
            return None if out is None else int(out)
        except Exception:
            _LSEG_PROVIDER_RUST_METRICS["coerce_int_fast_failures"] += 1
    _LSEG_PROVIDER_RUST_METRICS["coerce_int_fallbacks"] += 1
    return _coerce_int_py(value)


def _parse_unresolved_identifiers(message: str) -> tuple[str, ...]:
    if _lm2011_rust is not None:
        try:
            out = _lm2011_rust.parse_lseg_unresolved_identifiers_value(message)
            _LSEG_PROVIDER_RUST_METRICS["unresolved_identifier_parse_fast_success"] += 1
            return tuple(str(identifier) for identifier in out)
        except Exception:
            _LSEG_PROVIDER_RUST_METRICS["unresolved_identifier_parse_fast_failures"] += 1
    _LSEG_PROVIDER_RUST_METRICS["unresolved_identifier_parse_fallbacks"] += 1
    return _parse_unresolved_identifiers_py(message)


def _parse_unresolved_identifiers_py(message: str) -> tuple[str, ...]:
    match = UNRESOLVED_IDENTIFIERS_RE.search(message)
    if match is not None:
        return _parse_identifier_list_py(match.group("identifiers"))
...
def _parse_identifier_list_py(raw_identifiers: str) -> tuple[str, ...]:
...
def _parse_identifier_list(raw_identifiers: str) -> tuple[str, ...]:
    if _lm2011_rust is not None:
        try:
            out = _lm2011_rust.parse_lseg_identifier_list_value(raw_identifiers)
            _LSEG_PROVIDER_RUST_METRICS["identifier_list_fast_success"] += 1
            return tuple(str(identifier) for identifier in out)
        except Exception:
            _LSEG_PROVIDER_RUST_METRICS["identifier_list_fast_failures"] += 1
    _LSEG_PROVIDER_RUST_METRICS["identifier_list_fallbacks"] += 1
    return _parse_identifier_list_py(raw_identifiers)


def _coerce_records_to_utf8_py(records: list[dict[Any, Any]]) -> list[dict[str, str | None]]:
    return [
        {
...
def _coerce_records_to_utf8(records: list[dict[Any, Any]]) -> list[dict[str, str | None]]:
    if _lm2011_rust is not None:
        try:
            raw_rows = _lm2011_rust.lseg_utf8_record_rows(records)
            _LSEG_PROVIDER_RUST_METRICS["utf8_record_rows_fast_success"] += 1
            return [dict(row) for row in raw_rows]
        except Exception:
            _LSEG_PROVIDER_RUST_METRICS["utf8_record_rows_fast_failures"] += 1
    _LSEG_PROVIDER_RUST_METRICS["utf8_record_rows_fallbacks"] += 1
    return _coerce_records_to_utf8_py(records)
...
def _coerce_pandas_frame_to_utf8(*, candidate: Any) -> pl.DataFrame:
    ...
    normalized_records = _coerce_records_to_utf8(records)
    return pl.DataFrame(
        normalized_records,
